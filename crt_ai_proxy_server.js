...
async function handleManagePositions(req, res) {
  ...
  try {
    ...
    const pyCode = [
      ...
      '# === PORTFOLIO LEVEL BASKET TP/SL/BE ===',
      '# Tum acik pozisyonlarin toplam karini hesapla; esikler asilirsa basket aksiyonu uygula.',
      'try:',
      ...
      '      continue',
      '    point=max(float(getattr(si,"point",0.00001) or 0.00001),0.00001)',
      '    # Zaten BE seviyesinde ya da daha iyiyse atla',
      '    if side=="LONG" and cur_sl>0 and cur_sl>=(entry-point*2):',
      '      continue',
      '    if side=="SHORT" and cur_sl>0 and cur_sl<=(entry+point*2):',
      '      continue',
      '    new_sl=entry',
